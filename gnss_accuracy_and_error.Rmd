...
## Outline
This resource will analyze the accuracy and error of location data collected along a walk through Belval, LUxembourg, using a GNSS device (Crosscall Trekker-M1 Core).

## Actual path
Below is a walked tour around Belval, starting in the east next to the furnaces, going along the shopping center, then to the Park to the west and returning, ending in front of the University of LUxembourg's Maison des Sciences Humaines (MSH). 
...
The process locates, with some error, the device dependent on satellite distances at a specific time.
...
In the figure above we see the GNSS device data following closely the taken path in the western area, but less so in the south-eastern half.

In the graph below we can see, temporally, that there is a lot of deviation from the taken path at the start and end of the walk.
...
PERHAPS IT'S ALSO DUE TO SOMETHING ELSE?
LET'S MAP SATELLITES COUNT.
LET'S MAP THE MOVEMENT SPEED.
...
m <- merge(ll, as, by.x="rid", by.y="llid", all.x=TRUE)
...
plot(m$dt, m$satused_filt, cex=0.5, xlab = "", ylab = "", xaxt='n', yaxt='n', col=3, type='l', lwd=2)
axis(4, at=0:3*4, labels = 0:3*4)
mtext(side=4, line = 2, "Number of satellite signals received (smoothed)", col=3)

```
It appears that the number of satellites signals received may decrease at times of higher error.
...
m <- merge(ll, mv, by.x="rid", by.y="llid", all.x=TRUE)
...
par(new=TRUE, mar=c(4,4,1,4))
plot(m$dt.x, m$spd_filt, cex=0.5, xlab = "", ylab = "", xaxt='n', yaxt='n', col=3, lwd=2, type='l')
axis(4, at=0:10*.6, labels = 0:10*.6)
